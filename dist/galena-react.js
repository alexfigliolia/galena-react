...
typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('object-assign')) :
typeof define === 'function' && define.amd ? define(['exports', 'object-assign'], factory) :
(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["galena-react"] = {}, global.require$$0));
...
function requireReact_development () {
	...
	(function (exports) {
		...
		var _assign = require$$0__default["default"];
 ...
 * hash-table indexing
 */
var AutoIncrementingID = /*#__PURE__*/function () {
  function AutoIncrementingID() {
    this.incrementor = 0;
  }

  var _proto = AutoIncrementingID.prototype;
  ...
  _createClass(AutoIncrementingID, [{
    key: "nextID",
    get:
    /**
     * Next ID
...
var Indexer = /*#__PURE__*/function () {
  function Indexer() {
    this.incrementor = new AutoIncrementingID();
    this.subscriptions = new Map();
  }

  var _proto = Indexer.prototype;
  ...
  _proto.add = function add(callback) {
    var ID = this.incrementor.nextID;
  ...
  _proto.execute = function execute(state) {
    var subscriptions = this.subscriptions.values();

    for (var _iterator = _createForOfIteratorHelperLoose(subscriptions), _step; !(_step = _iterator()).done;) {
      var sub = _step.value;
      sub(state);
    }
  };
...
var Emitter = /*#__PURE__*/function () {
   ...
   */
  Emitter.emit = function emit(event, state) {
    var subscriptions = this.subscriptions.get(event);
  ...
  Emitter.subscribe = function subscribe(event, callback) {
    var subscriptions = this.subscriptions.get(event) || new Indexer();
    var ID = subscriptions.add(callback);
  ...
  Emitter.unsubscribe = function unsubscribe(event, ID) {
    var subscriptions = this.subscriptions.get(event);
  ...
  Emitter.destroy = function destroy() {
    this.subscriptions = new Map();
  };

  return Emitter;
}();
Emitter.subscriptions = new Map();
...
var Empire = /*#__PURE__*/function () {
   ...
   */
  Empire.getState = function getState(name) {
  ...
  Empire.add = function add(key, state) {
  ...
  Empire.subscribe = function subscribe(func) {
    var _this = this;

    var ID = this.subscriptionID.nextID;
    var IDs = [];

    for (var _iterator = _createForOfIteratorHelperLoose(this._state), _step; !(_step = _iterator()).done;) {
      var _step$value = _step.value,
          name = _step$value[0],
          State = _step$value[1];
      IDs.push({
        name: name,
        ID: State.subscribe(function () {
          func(_this._state);
        })
      });
    }
  ...
  Empire.unsubscribe = function unsubscribe(ID) {
    var _this2 = this;

    var IDs = this.subscriptions.get(ID) || [];
    IDs.forEach(function (_ref) {
      var name = _ref.name,
          ID = _ref.ID;

      var state = _this2.getState(name);
  ...
  Empire.destroy = function destroy() {
    this._state = new Map();
...
Empire._state = new Map();
Empire.stateUpdateID = new AutoIncrementingID();
Empire.subscriptionID = new AutoIncrementingID();
Empire.subscriptions = new Map();
...
function useGalenaState(stateName, connect, defaultValue) {
  var _useState = react.exports.useState(connect(Empire.getState(stateName).state || defaultValue)),
      state = _useState[0],
...
function useEmpireState(connect) {
  var _useState = react.exports.useState(connect(Empire.state)),
      state = _useState[0],
      setState = _useState[1];
...
class ContextFactory {
    ...
    constructor(stateName) {
        ...
        this.Context = react.exports.createContext(Empire.getState(stateName).state);
    ...
    createProvider(stateName) {
        const { Context } = this;
        return class ProviderFactory extends react.exports.Component {
            mounted = true;
            subscription;
            constructor(props) {
                super(props);
